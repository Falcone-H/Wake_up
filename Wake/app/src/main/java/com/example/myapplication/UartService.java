...
public class UartService extends Service {
    ...
    private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) throws UnsupportedEncodingException {
        ...
        if (TX_CHAR_UUID.equals(characteristic.getUuid())) {
            ...
            text = message + "--" + text;
            Log.d("Connect", "msg: " + text);
            intent.putExtra(EXTRA_DATA, text.getBytes());
        }
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
    }
    ...
    String message = null;
    public void writeRXCharacteristic(byte[] value) throws UnsupportedEncodingException {
        BluetoothGattService RxService = bluetoothGatt.getService(RX_SERVICE_UUID);
        if (RxService == null) {
            Log.d("Connect", "service_uuid");
            broadcastUpdate(DEVICE_DOES_NOT_SUPPORT_UART);
            return;
        }
        BluetoothGattCharacteristic RxChar = RxService.getCharacteristic(RX_CHAR_UUID);
        if (RxChar == null) {
            Log.d("Connect", "char_uuid");
            broadcastUpdate(DEVICE_DOES_NOT_SUPPORT_UART);
            return;
        }
        RxChar.setValue(value);
        message = new String(value, "UTF-8");
        boolean status = bluetoothGatt.writeCharacteristic(RxChar);
        Log.d("Connect", "write data");
    }
